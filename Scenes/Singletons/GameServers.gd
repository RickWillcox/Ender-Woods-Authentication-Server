...
func _Peer_Disconnected(gameserver_id):
	print("Game Server: %d Disconnected\n" % [gameserver_id])		
...
#	 that player will then be allowed to make a change / read that data in the database
	...
	print("Received Player Auth token, PlayerID: %s | Auth Token: %s\n" % [player_id, token])
...
remote func update_inventory(session_token, inventory):
	print(["Update new inventory{30}: ", str(inventory).left(30), " | Session Token{5}: ", str(session_token).left(5), "\n"])
